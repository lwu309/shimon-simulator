...
@with_goto
def checkinstructionlist(instructionlist, startpositions=[shimon.arm.positiontable[0], shimon.arm.positiontable[2], shimon.arm.positiontable[-3], shimon.arm.positiontable[-1]], strikercommands=None, midifilename=None, logfilename='info.log'):
    ...
    if strikercommands is not None and len(strikercommands) > 0:
        ...
        for striker in strikers:
            print(striker.number, striker.instructionqueue)
        """
    ...
    for time in range(totaltime):
            ...
            if len(arm.instructionqueue) > 0:
                ...
                # Motion of constant speed
                    ...
                    if len(arm.instructionqueue) > 1 and time == arm.nextinstructiontime():
                        ...
                        if abs(newposition - arm.position) - abs(decelerateposition - arm.position) >= sys.float_info.epsilon:
        ...
        if arms[1].position - arms[0].position < 15.0:
            print("Arm 1 collides with arm 2 at", time, "ms", file=log)
            return
        elif arms[2].position - arms[1].position < 55.5:
            print("Arm 2 collides with arm 3 at", time, "ms", file=log)
            return
        elif arms[3].position - arms[2].position < 15.0:
            print("Arm 3 collides with arm 4 at", time, "ms", file=log)
            return
        elif arms[0].position < shimon.arm.positiontable[0]:
            print("Arm 1 moves beyond the rail at", time, "ms", file=log)
            print(len(arms[0].instructionqueue), len(arms[1].instructionqueue), len(arms[2].instructionqueue), len(arms[3].instructionqueue))
            return
        elif arms[3].position > shimon.arm.positiontable[-1]:
            print(f"Arm 4 moves beyond the rail {shimon.arm.positiontable[-1]} at {time} ms", file=log)
            print(len(arms[0].instructionqueue), len(arms[1].instructionqueue), len(arms[2].instructionqueue), len(arms[3].instructionqueue))
            return
                        ...
                        if time == striker.instructionqueue[0] + 85:
                            ...
                            if (striker.lasthittime == -1):
                                ...
                                # Otherwise reset the counter
                                    ...
                                    striker.deadcounter = 1
                            if not striker.isdead:
                                loginfo(f'Striker {striker.number} hits the keyboard at time {time} on note {checkmidi.midimapping[int(round(arms[striker.armnumber - 1].position))]}')
                            ...
                            if midifilename is not None:
                                if len(notelist) == 0:
                                    ...
                                    if time - offset > max(notelist[0][0][0]):
                                        notelist.pop(0)
                                    isvalid, matchednote = checkmidi.isnotevalid(time - offset, checkmidi.midimapping[int(round(arms[striker.armnumber - 1].position))], notelist[0])
                                    if not isvalid:
                                        loginfo(f'Striker {striker.number} hits a wrong note ({checkmidi.midimapping[int(round(arms[striker.armnumber - 1].position))]}) or at a wrong time ({time - offset}) compared to the MIDI file')
                                    else:
